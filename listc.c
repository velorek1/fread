 ...
 * horizontal scroll.
   ...
   Last modified : 29/11/2024 + Horizontal option implementation started
   			      + ESC_KEY added
...
//DISPLAY INFO
     ...
     // write_str(1,1,path,B_CYAN,F_WHITE);
...
char selectorMenu(LISTCHOICE * aux, SCROLLDATA * scrollData) {
  ...
  while(control != CONTINUE_SCROLL) {
      ...
      if((listrows != newrows) || (listcolumns!=newcolumns)) {
	      //check terminal dimensions change; close if they are different
             scrollData->itemIndex = -1;double_escape = 1; scrollData->screenChanged = 1; ch = K_ESCAPE; scrollData-> lastch = K_ESCAPE; break;
        }
	...
	if (ch == K_ENTER || ch == K_ENTER2 || ch==K_SPACE || ch==K_TAB || ch =='a' || ch=='h')	// enter key
	{
...
if (locked == LOCKED) {    
  if(list_length > scrollData->displayLimit && scrollLimit > 0
	...
	if (ch == 'h') {scrollData->lastch = 'h'; break;}
    } while(ch != K_ENTER);
